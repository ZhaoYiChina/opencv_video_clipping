...
def main():
    ...
    video_caputre = cv.VideoCapture("C:\\Users\\Administrator\\Documents\\\My_Documents\\project\\opencv_video_clipping\\2.mov")
    ...
    size = (int(width), int(height * 0.75))

    # 创建视频写入对象
    videp_write = cv.VideoWriter("videoFrameTarget.mov", cv.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps, size)
    ...
    while success and not cv.waitKey(1) == 27:  # 读完退出或者按下 esc 退出
        ...
        frame_target = frame_src[150:int(height *0.75 + 150), 0:int(width)]
        # 写入视频文件
